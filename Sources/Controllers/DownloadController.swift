...
final class DownloadController: UITableViewController {
    private lazy var episodesRepository: DownloadEpisodesRepository =
        LocalDownloadEpisodesRepository { [weak self] (insertedIndexPaths, deletedIndexPaths, updatedIndexPaths) in
    ...
    private func setupObverservers() {
        NotificationCenter.default.addObserver(self,
                                               selector: #selector(handleEpisodeDownloadProgress(notification:)),
                                               name: .episodeDownloadProgress,
                                               object: nil)
    }

    @objc
    private func handleEpisodeDownloadProgress(notification: Notification) {
        guard let userInfo = notification.userInfo,
            let episodeId = userInfo["id"] as? String,
            let progress = userInfo["progress"] as? Float,
            let totalSize = userInfo["totalSize"] as? String else { return }

        DispatchQueue.main.async { [weak self] in
            guard let strongSelf = self else { return }
            guard let index = strongSelf.episodesRepository.objectIndex(by: episodeId),
                let cell = strongSelf.tableView
                    .cellForRow(at: IndexPath(row: index, section: 0)) as? EpisodeCell else { return }
...
extension DownloadController {
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if episode.getFileUrl() != nil {
            ...
            let alertController = UIAlertController(title: "File not found",
                                                    message: "Can not find the local file.",
                                                    preferredStyle: .actionSheet)
            alertController.addAction(UIAlertAction(title: "Play by stream URL",
                                                    style: .default) { (_) in
    ...
    private func pauseAndResumeDownload(_ episode: Episode, _ alertController: UIAlertController) {
    ...
    private func retryDownload(_ episode: Episode, _ alertController: UIAlertController) {
    ...
    private func cancelDownload(_ episode: Episode, _ alertController: UIAlertController) {
        if EpisodeDownloadManager.shared.isDonwloadInProgress(episode) {
