...
final class LocalDownloadEpisodesRepository: NSObject, DownloadEpisodesRepository {
    ...
    func objectIndex(by code: String) -> Int? {
        return fetchedResultsController.fetchedObjects?.firstIndex(where: { $0.code == code })
    }

    func object(by code: String) -> Episode? {
        return fetchedResultsController.fetchedObjects?.first(where: { $0.code == code })
    }
