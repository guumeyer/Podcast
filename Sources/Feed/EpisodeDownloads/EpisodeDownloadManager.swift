...
final class EpisodeDownloadManager {
    ...
    func isDonwloadInProgress(_ episode: Episode) -> Bool {
        guard let streamUrl = URL(string: episode.mediaUrl),
            let _ = activeDownloads[streamUrl] else { return false }
        return true
    }
    ...
    private func progressCompletionHandler(completion: ProgressCompletionHandlerType? = nil) -> ProgressCompletionHandlerType {
    ...
    fileprivate func updateEpisode(_ streamUrl: URL, _ destinationURL: URL) {
