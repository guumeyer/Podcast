...
extension PlayerView {
    ...
    private func changeTrack(_ move: PlayerMoveForward) {
        guard playList.count > 1,
            let currentEpisodeIndex = playList.firstIndex(where: { $0.code == episode.code }) else { return }
...
extension PlayerView {
    ...
    @objc
    private func handleAudioRouteChanged(note: Notification) {
...
extension PlayerView {
    ...
    @objc
    private func handlePan(gestore: UIPanGestureRecognizer) {
    ...
    @objc
    private func handlePanDismissal(gestore: UIPanGestureRecognizer) {
        switch gestore.state {
        case .changed:
    ...
    @objc
    private func handleTapMaximizePlayer() {
...
extension PlayerView {
    ...
    @objc
    private func handleCommanderCenterNextTrack() {
        print("CommanderCenter: NextTrack")
        changeTrack(.next)
    }

    @objc
    private func handleCommanderCenterPreviewTrack() {
        print("CommanderCenter: PreviewTrack")
        changeTrack(.preview)
    }

    @objc
    private func handleCommanderCenterPlayTrack() {
        print("CommanderCenter: Play")
        handlePlayAction()
    }

    @objc
    private func handleCommanderCenterPauseTrack() {
        print("CommanderCenter: Pause")
        handlePauseAction()
    }

    @objc
    private func handleCommanderCenterTogglePlayPause() {
        // Headset button
...
extension PlayerView {
    ...
    @objc
    private func handlePlayAndPause() {
